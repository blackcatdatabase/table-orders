...
namespace BlackCat\Database\Packages\Orders\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Orders\Definitions;
use BlackCat\Database\Packages\Orders\Criteria;
use BlackCat\Database\Packages\Orders\Dto\OrderDto as Dto;
use BlackCat\Database\Packages\Orders\Mapper\OrderDtoMapper as RowMapper;
...
final class OrderRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Orders\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'uuid' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Orders\Definitions::table());
            return $bulk->upsertMany($rows, [ 'uuid' ], [ 'uuid_bin', 'public_order_no', 'user_id', 'status', 'encrypted_customer_blob', 'encrypted_customer_blob_key_version', 'encryption_meta', 'currency', 'metadata', 'subtotal', 'discount_total', 'tax_total', 'total', 'payment_method' ]);
        }
    ...
    */
    ...
    public function getByUuidBin(string $uuidBin, bool $asDto = false): array|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null {
        $row = $this->getByUnique([ 'uuid_bin' => $uuidBin ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Orders\Mapper\OrderDtoMapper::fromRow($row);
    }
    ...
    public function getByTenantIdAndPublicOrderNo(int $tenantId, string $publicOrderNo, bool $asDto = false): array|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'public_order_no' => $publicOrderNo ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Orders\Mapper\OrderDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndPublicOrderNo(int $tenantId, string $publicOrderNo): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'public_order_no') . ' = :uniq_public_order_no';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_public_order_no' => $publicOrderNo ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndPublicOrderNo(int $tenantId, string $publicOrderNo) {
        $row = $this->getByTenantIdAndPublicOrderNo($tenantId, $publicOrderNo);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null {
    ...
    public function getByUuid(string $uuid, bool $asDto = false): array|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null {
        $row = $this->getByUnique([ 'uuid' => $uuid ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Orders\Mapper\OrderDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null {
