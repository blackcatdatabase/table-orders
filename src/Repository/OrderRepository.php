...
use BlackCat\Database\Packages\Orders\Repository\OrderRepositoryInterface;
...
class OrderRepository implements OrderRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'uuid' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Orders\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'uuid_bin', 'public_order_no', 'user_id', 'status', 'encrypted_customer_blob', 'encrypted_customer_blob_key_version', 'encryption_meta', 'currency', 'metadata', 'subtotal', 'discount_total', 'tax_total', 'total', 'payment_method' ]);
          return count($rows);
        }
    ...
    public function getIdByTenantIdAndPublicOrderNo(int $tenantId, string $publicOrderNo) {
        $row = $this->getByTenantIdAndPublicOrderNo($tenantId, $publicOrderNo, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Orders\Dto\OrderDto|null */
